...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // ===================================  RV32I instrcuctions ===================================================
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = SEXT(imm, 20));
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + (sword_t)(SEXT(imm, 20) << 12));
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + (sword_t)SEXT(imm, 20); check_jal(s->pc, s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + (sword_t)SEXT(imm, 12)) & ~1; R(rd) = s->snpc; check_jalr(s->pc, s->dnpc, rd, BITS(s->isa.inst.val, 19, 15), (sword_t)SEXT(imm, 12)));
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, R(rd) = Mr(src1 + (int64_t)SEXT(imm, 12), 4));
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, R(rd) = Mr(src1 + (int64_t)SEXT(imm, 12), 8));
  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, Mw(src1 + (int64_t)SEXT(imm, 7), 8, src2));

  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = SEXT(src1 + (int64_t)SEXT(imm, 12), 32));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = (int64_t)SEXT(src1, 32) >> BITS(imm, 4, 0));
