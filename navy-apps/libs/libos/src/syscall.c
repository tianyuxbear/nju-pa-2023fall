...
intptr_t _syscall_(uint64_t type, uint64_t a0, uint64_t a1, uint64_t a2) {
...
int _open(const char *path, int flags, mode_t mode) {
  int fd = _syscall_(SYS_open, (uint64_t)path, flags, mode);
  return fd;
}

int _read(int fd, void *buf, size_t count) {
  int bytes = _syscall_(SYS_read, fd, (uint64_t)buf, count);
  return bytes;
}

int _write(int fd, void *buf, size_t count) {
  int bytes = _syscall_(SYS_write, fd, (uint64_t)buf, count);
  return bytes;
}
...
void *_sbrk(intptr_t increment) {
  ...
  uint64_t addr = pre_program_break + increment;
  int ret = _syscall_(SYS_brk, addr, 0, 0);
  if(ret == 0){
    cur_program_break = addr;
...
int _execve(const char *fname, char * const argv[], char *const envp[]) {
  printf("fname: %p    argv: %p    envp: %p\n", (uint64_t)fname, (uint64_t)argv, (uint64_t)envp);
  int ret = _syscall_(SYS_execve, (uint64_t)fname, (uint64_t)argv, (uint64_t)envp);
  return ret;
}
